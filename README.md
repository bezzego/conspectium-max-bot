...
3. **Запуск бота:**
   ```bash
   # Запуск сервера с ботом (бот работает через webhook)
   poetry run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
   ```
   
   После запуска сервера бот автоматически обрабатывает webhook обновления от Max. 
   При команде `/start` бот отправляет приветственное сообщение с кнопкой для открытия мини-приложения.

4. При генерации конспектов по аудио используется основной ключ Gemini, а при создании конспектов из текста — 
   `GOOGLE_API_KEY_TEXT`.

### Новые возможности

- **Выбор количества вопросов в тесте**: При создании теста из конспекта теперь можно выбрать количество вопросов (от 1 до 20) с помощью ползунка. По умолчанию создается 5 вопросов.
